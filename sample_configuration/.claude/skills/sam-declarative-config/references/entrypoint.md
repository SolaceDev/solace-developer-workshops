...
running entrypoint with `sam config pull --only entrypoint --name X`) for
...
public host, where `<slug>` is the entrypoint's URL-stable slug (derived
...
to the slug survives reprovisioning. **Never put the entrypoint UUID in an
...
means the slug/path is wrong. The slug only exists after the entrypoint is
created, so the order is: apply the entrypoint → read its slug → set the
provider URL.
...
manifest's `resources.agents`), or unmatched messages 404 at dispatch.
...
|---|---|---|---|---|---|
| `mailbox` | `string` | yes |  | matches regex | The email address whose inbox this entrypoint polls. e.g. support@solace.com |
...
| `allowed_senders` | `string` |  |  |  | Comma-separated list of sender email addresses permitted to trigger the entrypoint. When empty, all senders are allowed (subject to the auth policy). When set, any sender not in the list is dropped before auth and identity evaluation. |
...
```yaml
kind: entrypoint
name: example_email_entrypoint
description: "Example email entrypoint. Replace with a real description (10+ chars)."
spec:
  type: email
  deploy: true
  values:
    ...
    auth_mode: "oauth2"
    imap_password: ${EXAMPLE_EMAIL_ENTRYPOINT_IMAP_PASSWORD}  # secret — provide via env var
    # optional: smtp_host: "smtp.gmail.com"
    # optional: smtp_port: 587
    oauth2_tenant_id: "..."
    oauth2_client_id: "..."
    oauth2_client_secret: ${EXAMPLE_EMAIL_ENTRYPOINT_OAUTH2_CLIENT_SECRET}  # secret — provide via env var
...
| `stateRedirectionEnabled` | `select` |  | `true` |  | When on, this entrypoint routes its OAuth callback through a single dispatcher entrypoint (the WebUI's /api/v1/auth/callback by default) instead of its own /gw/<slug>/oauth/callback, so the IdP only needs one redirect URI allowlisted for every MCP entrypoint. Leave the dispatcher URL below blank to auto-derive it from the cluster WebUI base. Turn off only for a self-managed IdP that allowlists each entrypoint's own callback. |
...
Connect your AI agent to WhatsApp via the WhatsApp Business Cloud API. Inbound messages arrive over a Meta webhook; replies are sent through the Graph API.

> ⚠ Requires a WhatsApp Business Account, a Cloud API phone number, and a Meta app with a webhook subscribed to the 'messages' field. Use a System User permanent access token in production. The Meta Graph API version is pinned by the entrypoint (currently v25.0) — no configuration needed.

| Field | Type | Required | Default | Validation | Description |
|---|---|---|---|---|---|
| `phone_number_id` | `string` | yes |  |  | The Cloud API phone-number ID the entrypoint sends from (the "From" number). Found in the Meta app's WhatsApp > API Setup. |
| `access_token` | `password (secret)` | yes |  | len 1–1000; secret | System User permanent token (or 24-hour dev token) used as the Bearer credential on outbound Graph API calls. Leave placeholder when editing to keep existing value. |
| `app_secret` | `password (secret)` | yes |  | len 1–500; secret | Meta App Secret, used to validate the X-Hub-Signature-256 HMAC on every inbound webhook POST. Leave placeholder when editing to keep existing value. |
| `verify_token` | `password (secret)` | yes |  | len 1–500; secret | Arbitrary string configured in the Meta webhook setup; echoed back during the one-time GET subscription handshake. Leave placeholder when editing to keep existing value. |
| `default_agent_name` | `agent_select` |  |  |  | The agent that handles inbound WhatsApp messages. Leave empty to route to the first discovered agent. |
