...
The Slack connector allows agents to send messages to Slack channels and update messages they have already posted.

## Overview

The Slack connector authenticates to a Slack workspace with a Bot User OAuth Token (`xoxb-`) and provides agents with two tools:

- A send-message tool that posts plain text, Slack Block Kit content, threaded replies, and file attachments
- An update tool that edits a message the agent has already posted

Agents call these tools to push notifications and keep long-running status messages current. The connector is outbound only—agents drive the conversation, and the connector does not receive messages from Slack. For inbound messages from Slack, see `link to "Slack Gateway"` .

### When to Use the Connector vs the Slack MCP Server
...
- Network connectivity from Agent Mesh to the Slack Web API

The following sections describe each prerequisite in detail.

### Slack App and Bot Token

You need a Slack app installed in your workspace, with a Bot User OAuth Token (`xoxb-`) from the **OAuth & Permissions** page. The same token can be used for both the Slack connector and the Slack gateway, provided the app has the scopes both require.
...
Ensure Agent Mesh can reach the Slack Web API over the network. Verify that firewalls and security groups allow outbound HTTPS traffic to `slack.com` and `*.slack.com`.

### Quick Setup Using a Slack App Manifest

The fastest way to create the Slack app is to paste a manifest into your workspace. The manifest combines the permissions the Slack connector requires with those the Slack gateway requires, so a single app can power both. The connector ignores the gateway-only event subscriptions and Socket Mode setting, and the gateway picks them up if you deploy it later. For information about the gateway, see `link to "Slack Gateway"` .

On the `link to "Slack apps page"` , click **Create New App**, choose **From a manifest**, select your workspace, and paste the following JSON.
...
After Slack creates the app, install it to your workspace from the **Install App** page and copy the Bot Token from the **OAuth & Permissions** page. If you also deploy the Slack gateway later, the gateway requires an additional App-Level Token. For more information, see `link to "Slack Gateway"` .

## Creating the Connector

Create the connector through the **Connectors** section in the Agent Mesh web interface, then fill in the following fields. Select **Slack** as the connector type. For the general flow of creating any connector, see `link to "Connectors"` .
...
| Description | Yes | 10–1000 characters. Helps the agent's language model decide when to invoke the Slack tools. Example: "Send change-approval requests and deployment notifications to the engineering Slack workspace" |
| Bot Token | Yes | Bot User OAuth Token from the Slack app. Must start with `xoxb-`. The form masks this value |

To preserve an existing token when you edit a saved connector, keep the placeholder value in the **Bot Token** field.

## Tools the Connector Provides

When you assign this connector to an agent, the agent gains access to the following tools:

- **Send Message**—sends a message to a Slack channel, direct message, or thread. The agent provides the channel and message text. The channel can be a channel ID (`C01234ABCDE`), a channel name (`#channel-name`), a user ID for a direct message, or a user's email address, which the connector resolves to a user ID through the Slack Web API. The agent can supply Block Kit JSON for rich formatting, a parent message timestamp to reply in a thread, and file attachments to upload alongside the message. Standard Markdown in the message text is converted to Slack `mrkdwn` format automatically, so bold, links, and headings render correctly without the agent producing Slack-specific syntax. Attachments and Block Kit content are mutually exclusive—the agent uses one or the other in a given call.
...
Slack connectors use a shared credential model. For more information, see `link to "Connectors"` .
...
- Limit which channels the bot is invited to
...
- The bot is invited to the channel with `/invite @bot` for private channels and public channels without `chat:write.public`
...
You have just created a Slack connector. Now you can assign the connector to an agent so it can use the Slack tools. For more information and instructions, see `link to "Connectors"` .
