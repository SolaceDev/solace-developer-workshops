...
| `general_agent_tools` | `get_current_time`, file→markdown converters, `ask_user_question` | |
| `image_tools` | image generate/describe/edit, audio describe | Alias: `builtin_image_tools`. |
| `data_analysis` | SQL over data, sqlite, JMESPath transform, merge, Plotly charts | Chart tool is STR-backed. |
...
- STR-backed tools (documents, media, charts, python) additionally require the deployment's STR fleet to host the matching binaries — if a tool silently does nothing in a trial environment, that's the first thing to check (hand off to `sam-operate` for diagnosis).
